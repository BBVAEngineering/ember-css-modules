...
module.exports = class ClassTransformPlugin {
  ...
  transformStatement(node) {
    ...
    } else if (node.path.original === 'context-class') {
      this.transformContextClassHelperInvocation(node);
    } else {
  ...
  transformLocalClassHelperInvocation(node) {
    ...
    node.hash.pairs.push(this.builders.pair('from', this.localStylesNode()));
  }

  // Transform {{local-class 'foo'}} into {{local-class 'foo' from='path/to/styles-module'}}
  transformContextClassHelperInvocation(node) {
    if (utils.getPair(node, 'from')) { return; }

    node.hash.pairs.push(this.builders.pair('from', this.contextStylesNode()));
  }

  transformPossibleComponentInvocation(node) {
    const transform = (key) => {
      let localClassPair = utils.getPair(node, `${key}-class`);
      ...
      utils.pushAll(params, this[`${key}ToPath`](localClassPair.value));
  ...
  transformElementNode(node) {
    const transform = (key) => {
      let localClassAttr = utils.getAttr(node, `${key}-class`);
      ...
      utils.pushAll(parts, this[`${key}ToPath`](localClassAttr.value));
  ...
  toPath(key, node) {
    if (~['SubExpression', 'MustacheStatement'].indexOf(node.type)) {
      return this.dynamicPath(key, node);
    } else if (node.type === 'ConcatStatement') {
      return this.concatPath(key, node);
    } else if (~['TextNode', 'StringLiteral'].indexOf(node.type)) {
      return this.staticPath(key, node);
    } else {
      throw new TypeError(`ember-css-modules - invalid type, ${node.type}, passed to ${key}-class attribute.`);
    }
  }

  localToPath(node) {
    return this.toPath('local', node);
  }

  contextToPath(node) {
    return this.toPath('context', node);
  }

  dynamicPath(key, node) {
    ...
    let hash = this.builders.hash([this.builders.pair('from', this[`${key}StylesNode`]())]);
  ...
  contextStylesNode() {
    return this.builders.path('this.__styles__');
  }
  ...
  concatPath(key, node) {
    ...
    return this.dynamicPath(key, concatStatement);
  }
  ...
  staticPath(key, node) {
    ...
    return [this.builders[exprBuilder](
      ...
      this.builders.hash([this.builders.pair('from', this[`${key}StylesNode`]())])
    )];
  }
